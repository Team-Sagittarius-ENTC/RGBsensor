...
void RGBsensor::update_coeff(byte ref_, byte itter){
  float beta = 0;
  float gamma = 0;
  float delta = 0;
  float theta = 0;
...
void RGBsensor::procRegression(byte itter){
  float ans[3]{};
  float inv[3][3]{};
  float SIG_x = 0;
  float SIG_xy = 0;
...
void RGBsensor::calibrate(Display Lcd, Keypad keypad, bool grayScale){
  ...
  */
  ...
  // if not the grayscale then we want the number of points that are going to give
  byte itter = 9;
  if (!grayScale){
    Lcd.clear_();
    Lcd.printMsg("Itter Pts -> ");
    Lcd.blink_(true);

    while(true){
      itter = keypad.read_key().toInt(); // input the number of the itteration points
      if(0 < itter && itter < 10) {
        Lcd.blink_(false);
        break;
      }
  ...
  for(byte i = 0; i < itter; ++i){
    ...
    if(grayScale){  
      ...
      int *color_ = Lcd.colorInputDisplay(keypad, false); // get the color from the user
      for (byte n = 0; n < 3; ++n){
        raw_data[i][n] = color_[n];
      }
    }
 
    while (keypad.read_key(false) != "#"){
      readColor(true);
      Lcd.printMsg(String(color[2]) + ", " + String(color[1]) + ", " + String(color[0]), 0, true); // printing the color      
    ...
    for (byte j = 0; j < 3; ++j){
      raw_data[i][j+3] = float(color[j]) / 100; //deviding the color and store it in the array
    }
  ...
  procRegression(itter); //Proccess the regression constants
